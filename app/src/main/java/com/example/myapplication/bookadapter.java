...
public class bookadapter extends FirestoreRecyclerAdapter<bookss,bookadapter.bookholder> {
    private onnameclicklistner listner;
     ...
     * @param options
     */
    public bookadapter(@NonNull FirestoreRecyclerOptions<bookss> options) {
        super(options);
    }

    @Override
    protected void onBindViewHolder(@NonNull bookholder holder, int position, @NonNull bookss model) {
        holder.tvname.setText(model.getName()+" WITH "+model.getDocname());
    ...
    public void setonitemclicklistner(onnameclicklistner listner){
